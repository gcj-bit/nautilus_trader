...
def test_rust_greeks_data_roundtrip(tmp_path):
    """
    Test Rust GreeksData roundtrip via catalog and from_delta from Python.
    """
    from nautilus_trader.core.nautilus_pyo3 import GreeksData
    from nautilus_trader.core.nautilus_pyo3 import InstrumentId
    from nautilus_trader.core.nautilus_pyo3 import ParquetDataCatalogV2
    from nautilus_trader.core.nautilus_pyo3.model import CustomData
    from nautilus_trader.core.nautilus_pyo3.model import DataType
    ...
    register_custom_data_class(GreeksData)
    catalog_path = tmp_path / "catalog_greeks"
    catalog_path.mkdir(parents=True, exist_ok=True)
    pyo3_catalog = ParquetDataCatalogV2(str(catalog_path))

    instrument_id = InstrumentId.from_str("ES.GLBX")
    metadata = {"venue": "TEST", "instrument_id": str(instrument_id)}
    data_type = DataType("GreeksData", metadata, str(instrument_id))

    # One instance via from_delta (Python classmethod)
    from_delta_item = GreeksData.from_delta(instrument_id, 0.5, ts_event=1000)
    assert from_delta_item.instrument_id == instrument_id
    assert from_delta_item.delta == 0.5
    assert from_delta_item.ts_event == 1000
    assert from_delta_item.ts_init == 1000

    # Second instance via from_delta with different args
    instrument_id2 = InstrumentId.from_str("NQ.GLBX")
    from_delta_item2 = GreeksData.from_delta(instrument_id2, -0.25, ts_event=2000)

    # One instance via full constructor (ts_event, ts_init, instrument_id, ...)
    full_item = GreeksData(
        3000,
        3000,
        instrument_id,
        True,
    ...
    original_data = [from_delta_item, from_delta_item2, full_item]
    data_type2 = DataType(
        "GreeksData",
        {"venue": "TEST", "instrument_id": str(instrument_id2)},
        str(instrument_id2),
    )
    wrapped = [
        CustomData(data_type, original_data[0]),
        CustomData(data_type2, original_data[1]),
        CustomData(data_type, original_data[2]),
    ]
    ...
    for item in result:
        assert isinstance(item, CustomData), f"Expected CustomData, found {type(item)}"
        inner = item.data
        assert isinstance(inner, GreeksData), f"Expected GreeksData in .data, found {type(inner)}"
        roundtripped.append(inner)
    ...
    for expected, actual in zip(original_data, roundtripped, strict=True):
        assert expected.instrument_id == actual.instrument_id
        assert expected.delta == actual.delta
        assert expected.ts_event == actual.ts_event
        assert expected.ts_init == actual.ts_init
        assert expected.is_call == actual.is_call
        assert expected.strike == actual.strike
        assert expected.expiry == actual.expiry
        assert expected.multiplier == actual.multiplier
        assert expected.quantity == actual.quantity
        assert expected.vol == actual.vol
        assert expected.price == actual.price
        assert expected.gamma == actual.gamma
        assert expected.vega == actual.vega
        assert expected.theta == actual.theta
        assert expected.itm_prob == actual.itm_prob
    ...
    _assert_custom_data_json_roundtrip(result, GreeksData, assert_greeks)
